...
const App = () => {
  ...
  const triggerEvent = (msg, type = "default") => {
    setFeedback(msg);
    setTimeout(() => setFeedback(""), 2000);
  };

  // --- Core Game Logic ---
  const spawnPiece = useCallback((forceType = null) => {
  ...
  const checkCollision = (piece, newPos, newGrid = grid) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'ArrowUp') rotate(activePiece);
      if (e.key === ' ') useMercy();
    };
  ...
  // --- Visual Styling ---
  const karmaFilter = {
    filter: `sepia(${karma}%) hue-rotate(${karma * 0.7}deg) contrast(${100 + karma}%)`,
    transform: karma > 85 ? `translate(${Math.random()*4-2}px, ${Math.random()*4-2}px)` : 'none'
  };

  return (
    <div className="min-h-screen bg-stone-950 text-stone-200 flex items-center justify-center p-4 font-serif select-none overflow-hidden">
      {/* Mandala Background Decoration */}
      <div className="absolute inset-0 opacity-5 pointer-events-none flex items-center justify-center">
        <div className="w-[800px] h-[800px] border-[40px] border-yellow-500 rounded-full animate-spin-slow" />
      </div>
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
        ...
        <div className="space-y-6">
          <div className="bg-stone-900/80 p-6 rounded-3xl border border-stone-800 backdrop-blur-md">
            <h1 className="text-5xl font-black text-yellow-600 mb-1 tracking-tighter italic">禪-TRIS</h1>
            <p className="text-stone-500 text-sm mb-6 uppercase tracking-widest">Digital Samsara Simulator</p>
            
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs mb-1 text-stone-400">
                  <span>현생의 업보 (Karma)</span>
                  <span className={karma > 80 ? "text-red-500 animate-pulse" : ""}>{Math.floor(karma)}%</span>
                </div>
                <div className="h-2 bg-stone-800 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-700 ${karma > 70 ? 'bg-red-600' : 'bg-yellow-500'}`} style={{ width: `${karma}%` }} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="h-2 bg-stone-800 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${(linesCleared/NIRVANA_GOAL)*100}%` }} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-stone-900/80 p-4 rounded-2xl border border-stone-800 text-center">
              <LucideSkull size={20} className="mx-auto mb-2 text-stone-600" />
              <div className="text-2xl font-bold">{samsaraCount}</div>
              <div className="text-[10px] text-stone-500 uppercase">윤회 횟수</div>
            </div>
            <div className="bg-stone-900/80 p-4 rounded-2xl border border-stone-800 text-center">
              <LucideSparkles size={20} className="mx-auto mb-2 text-blue-400" />
              <div className="text-2xl font-bold text-blue-400">{merit}</div>
              <div className="text-[10px] text-stone-500 uppercase">누적 공덕</div>
            </div>
          </div>
          ...
          <button 
            onClick={useMercy}
            disabled={merit < 300}
            className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl ${merit >= 300 ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-stone-800 text-stone-600 cursor-not-allowed'}`}
          >
            <LucideHeart size={20}/> 자비의 손길 (300 공덕)
          </button>
        </div>
        ...
        <div className="relative group">
          {activeGuardian && (
              <div className="absolute -top-12 left-0 right-0 flex items-center justify-center gap-2 animate-bounce bg-stone-900 py-2 rounded-full border border-stone-700 shadow-xl z-20">
                  {GUARDIANS[activeGuardian].icon}
                  <span className="text-sm font-bold text-white">{GUARDIANS[activeGuardian].name}의 가호 ({guardianTimer}s)</span>
              </div>
          )}

          <div className="p-1 bg-stone-800 rounded-xl border-8 border-stone-900 shadow-[0_0_50px_rgba(0,0,0,1)] relative overflow-hidden">
            <div 
              className="grid grid-cols-10 grid-rows-20 gap-px bg-stone-950/50" 
              style={{ width: '320px', height: '640px', ...karmaFilter }}
            >
              {grid.map((row, y) => 
                row.map((cell, x) => {
                  let color = cell || 'bg-stone-900/20';
                  if (activePiece) {
                  ...
                  return (
                    <div 
                      key={`${x}-${y}`} 
                      className={`${color} rounded-[1px] transition-all duration-150 ${cell ? 'shadow-[inset_0_0_8px_rgba(255,255,255,0.2)] border border-white/5' : ''}`}
                    />
                  );
                })
              )}
            ...
            {(gameOver || isNirvana) && (
              <div className="absolute inset-0 bg-stone-950/90 backdrop-blur-md flex flex-col items-center justify-center text-center p-8 z-30">
                {isNirvana ? (
                  <div className="animate-in fade-in zoom-in duration-1000">
                    <div className="w-24 h-24 bg-yellow-500 rounded-full blur-2xl absolute opacity-50 animate-pulse" />
                    <LucideSun size={80} className="text-yellow-400 mb-6 relative animate-spin-slow"/>
                    <h2 className="text-4xl font-black mb-4 text-yellow-500 tracking-tighter">아뇩다라삼먁삼보리</h2>
                  ...
                  <>
                    <LucideSkull size={64} className="text-red-700 mb-6 animate-pulse"/>
                ...
                <button 
                  ...
                  className="bg-white text-black px-8 py-3 rounded-full font-black flex items-center gap-2 hover:scale-105 transition-transform"
                >
          ...
          <div className="mt-4 text-center py-2 h-10 text-yellow-600 font-bold animate-pulse text-lg">
            {feedback}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-stone-900/80 p-6 rounded-3xl border border-stone-800">
            <h3 className="text-stone-400 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                <LucideShield size={16}/> 사천왕의 가호 (4줄 제거 시)
            </h3>
            <div className="space-y-4">
                {Object.entries(GUARDIANS).map(([key, g]) => (
                    <div key={key} className={`flex gap-3 p-3 rounded-xl border transition-all ${activeGuardian === key ? 'bg-stone-800 border-yellow-600 scale-105 shadow-lg' : 'border-transparent opacity-40'}`}>
                        <div className="mt-1">{g.icon}</div>
                        <div>
                            <div className="text-sm font-bold text-stone-200">{g.name}</div>
                            <div className="text-[10px] text-stone-500 leading-tight">{g.desc}</div>
                        </div>
                    </div>
                ))}
            </div>
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-br from-stone-900 to-stone-950 border border-stone-800 italic text-stone-500 text-xs leading-relaxed space-y-2">
